...
for i in range(len(testIn)):
  ...
  if studentAns != testOut[i]:
    ...
    print("Remember - the answer should be in pounds not pence.")
    sys.exit(1)
...
for i in range(len(testIn)):
  ...
  if studentAns != testOut[i]:
    ...
    print("Expected: " + str(testOut[i]))
    print("Received: " + str(studentAns))
    print("Note that the first 10 miles are always at 50p, it is only subsequent miles at the lower price.")
    sys.exit(1)    
